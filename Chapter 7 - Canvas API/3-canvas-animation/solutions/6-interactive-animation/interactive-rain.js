...
class Drop {
  ...
  update() {
    // Change drift also according to mouse position
    this.x += this.drift + mouseDrift;
...
// Change rain when if moves mouse

function changeRain(event) {
  mouseDrift = (event.offsetX - 512) / 50;
  newDropsPerFrame = event.offsetY / 20  + 1;
}

canvas.addEventListener('mousemove', changeRain);
