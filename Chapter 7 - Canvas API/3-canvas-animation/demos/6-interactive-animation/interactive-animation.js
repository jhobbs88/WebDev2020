...
class Bubble {
  ...
  draw() {
    ...
    this.path.arc(this.positionX, this.positionY, this.radius, 0, Math.PI * 2);
    ctx.fill(this.path);
...
function popBubble(event) {
  for (let i = bubbles.length - 1; i >= 0; i--) {
    if (
      ctx.isPointInPath(bubbles[i].path, event.offsetX, event.offsetY)
      || ctx.isPointInStroke(bubbles[i].path, event.offsetX, event.offsetY)
    ) {
      bubbles.splice(i, 1);
    }
  }
